...
2. `sm__sass_inst_executed_op_shared_ld`, ncu中解释为`of warp instructions executed: LDS, LD`，LDS, LD指令执行的次数。需要注意的是指令是以warp为单位执行的。
...
采集计数器后，暴露出一些问题。虽然按照之前的思路计算ratio，**kernel size为5时的ratio确实比kernel size为3时的小**，但是kernel size=3时计算出的ratio=1确实让人迷惑。
